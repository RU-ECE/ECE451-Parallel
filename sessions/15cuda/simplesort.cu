...
void quiksort(rec x[], u32 L, u32 R) {
    ...
    if (i > L) quiksort(x, L, i - 1);
    if (i + 1 < R) quiksort(x, i + 1, R);
}
...
__global__ void merge(rec x[], rec y[], u32 n, u32 sorted_size) {
    ...
    u32 num_pairs = n / (2 * sorted_size);

    if (thread_id >= num_pairs) return;

    u32 left_start = thread_id * 2 * sorted_size;
    u32 right_start = left_start + sorted_size;
    u32 output_start = left_start;
    
    u32 left_end = right_start;
    u32 right_end = (right_start + sorted_size < n) ? right_start + sorted_size : n;
...
/*
    ...
    Once we see how this works, we can do this in bigger groups (16, 32, 64). We could also try to have multiple threads working on the
    same optimal sorting network.
*/
__global__ void sortingnetwork8(rec* x, rec* y, u32 n) {
    u32 thread_id = blockIdx.x * blockDim.x + threadIdx.x;
    u32 num_groups = n / 8;

    if (thread_id >= num_groups) return; // we only need this many threads
...
/*  
    ...
    [(1,2),(3,4),(5,6)]
*/
    sort2(a, c);    sort2(e, g);    sort2(b, d);    sort2(f, h);
...
__global__ void merge4(rec x[], rec y[], u32 n, u32 sorted_size) {
    u32 thread_id = blockIdx.x * blockDim.x + threadIdx.x;
    u32 num_quads = n / (4 * sorted_size);

    if (thread_id >= num_quads) return;

    u32 g0 = thread_id * 4 * sorted_size;
    u32 g1 = g0 + sorted_size;
    u32 g2 = g1 + sorted_size;
    u32 g3 = g2 + sorted_size;
    u32 g_end = (g3 + sorted_size < n) ? g3 + sorted_size : n;
    ...
    while (i < i_end && j < j_end && k < k_end && m < m_end) {
        ...
        if (min_idx == 0) y[out++] = x[i++];
        else if (min_idx == 1) y[out++] = x[j++];
        else if (min_idx == 2) y[out++] = x[k++];
        else y[out++] = x[m++];
    }
    // finish remaining elements
    while (i < i_end && j < j_end && k < k_end) {
        if (x[i].key < x[j].key) {
            if (x[i].key < x[k].key) y[out++] = x[i++];
            else y[out++] = x[k++];
        } else {
            if (x[j].key < x[k].key) y[out++] = x[j++];
            else y[out++] = x[k++];
        }
    }
    while (i < i_end && j < j_end && m < m_end) {
        if (x[i].key < x[j].key) {
            if (x[i].key < x[m].key) y[out++] = x[i++];
            else y[out++] = x[m++];
        } else {
            if (x[j].key < x[m].key) y[out++] = x[j++];
            else y[out++] = x[m++];
        }
    }
    ...
    while (i < i_end) y[out++] = x[i++];
    while (j < j_end) y[out++] = x[j++];
    while (k < k_end) y[out++] = x[k++];
    while (m < m_end) y[out++] = x[m++];
}
...
__global__ void initial_sort(rec x[], u32 n, u32 block_size) {
    u32 thread_id = blockIdx.x * blockDim.x + threadIdx.x;
    u32 start = thread_id * block_size;
    if (start >= n) return;
    u32 end = (start + block_size < n) ? start + block_size : n;
...
void merge_sort_wrapper(void* ptr, unsigned int n) {
    ...
    const u32 initial_block_size = 256;
    u32 num_initial_blocks = (n + initial_block_size - 1) / initial_block_size;
    u32 num_blocks_initial = (num_initial_blocks + num_threads_global - 1) / num_threads_global;
    if (num_blocks_initial == 0) num_blocks_initial = 1;
...
void merge_sort2_wrapper(void* ptr, unsigned int n) {
    ...
    if (num_groups > 0) {
        ...
        if (num_blocks_sort == 0) num_blocks_sort = 1;
...
/**
    ...
    copy the results back and make sure the original values are there.
*/
const u32 n = 16*1024*1024; // 64M elements, 512MB data (each element is 8 bytes)
rec* y_gpu_global;
...
int main() {
    ...
    err = cudaMalloc((void**)&x_gpu, bytes);
    if (err != cudaSuccess) {
    ...
    err = cudaMalloc((void**)&y_gpu, bytes);
    if (err != cudaSuccess) {
