...
float horizontal_sum(const __m256 v) {
	...
	auto temp = _mm256_hadd_ps(v, v); //  [A+B, C+D, E+F, G+H, A+B, C+D, E+F, G+H]
	// Step 2: Reduce further by swapping and adding across lanes
	const auto low = _mm256_castps256_ps128(temp); // Lower 128 bits: [A+B, C+D, E+F, G+H]
	const auto high = _mm256_extractf128_ps(temp, 1); // Upper 128 bits: [A+B, C+D, E+F, G+H]
	auto result = _mm_add_ps(low, high); // [A+B+C+D, E+F+G+H, unused, unused]
