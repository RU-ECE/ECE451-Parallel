...
float dot_simd2(const float a[], const float b[], int n) {
    float sum = 0.0f;
...
float dot_simd3(const float a[], const float b[], int n) {
    float sum = 0.0f;
...
float prod(const float a[], int n) {
    float prod = 1.0f;
...
float horizontal_sum(__m256 v) {
    ...
    __m128 low = _mm256_castps256_ps128(temp); // Lower 128 bits: [A+B, C+D, E+F, G+H]
    __m128 high = _mm256_extractf128_ps(temp, 1); // Upper 128 bits: [A+B, C+D, E+F, G+H]
...
*/
float dot_avx2manual(const float a[], const float b[], int n) {
