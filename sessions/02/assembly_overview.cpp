...
uint64_t uint64_math(const uint64_t a, const uint64_t b) {
	const uint64_t p = a + b;
	const uint64_t q = a - b;
	const uint64_t r = a * b;
	const uint64_t s = a / b;
	const uint64_t w = a % b;
	const uint64_t x = -a;
...
int main(int argc, char* argv[]) {
	const uint64_t r = uint64_math(1, 2); // this will probably be optimized out at compile time
	...
	}
	{
		...
		t3.join();
		t4.join();
		const uint64_t count = count1 + count2 + count3 + count4;
		const auto end = high_resolution_clock::now();
		cout << "prime count=" << count << " elapsed: " << duration_cast<microseconds>(end - start).count() * 1e-6
			 << endl;
	}
