...
long timer_end(const struct timespec start_time) {
	struct timespec end_time;
	clock_gettime(CLOCK_MONOTONIC, &end_time);
	long diffInNanos =
		1000000000 * end_time.tv_sec + end_time.tv_nsec - (1000000000 * start_time.tv_sec + start_time.tv_nsec);
...
/*
  ...
  maxTextureCubemapLayered = {32768, 2046}, maxSurface1D = 32768, maxSurface2D = {131072, 65536}, maxSurface3D = {16384,
  16384, 16384}, maxSurface1DLayered = {32768, 2048}, maxSurface2DLayered = {32768, 32768, 2048}, maxSurfaceCubemap =
  32768, maxSurfaceCubemapLayered = {32768, 2046}, surfaceAlignment = 512, concurrentKernels = 1, ECCEnabled = 0,
  pciBusID = 33,
  pciDeviceID = 0,
  pciDomainID = 0,
  tccDriver = 0,
  ...
  managedMemory = 1,
  isMultiGpuBoard = 0,
  multiGpuBoardGroupID = 0, hostNativeAtomicSupported = 0, singleToDoublePrecisionPerfRatio = 32, pageableMemoryAccess =
  0, concurrentManagedAccess = 1, computePreemptionSupported = 1, canUseHostPointerForRegisteredMem = 1,
  cooperativeLaunch = 1, cooperativeMultiDeviceLaunch = 1, sharedMemPerBlockOptin = 49152,
  pageableMemoryAccessUsesHostPageTables = 0, directManagedMemAccessFromHost = 0, maxBlocksPerMultiProcessor = 32,
  accessPolicyMaxWindowSize = 0, reservedSharedMemPerBlock = 0}
*/

void dump(cudaDeviceProp prop) {
	cout << "Display CUDA device properties\n";
	u64 uuid = *static_cast<u64*>(&prop.uuid);
