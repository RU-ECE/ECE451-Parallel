...
void gen_sequential(rec x[], const unsigned int n) {
	for (auto i = 0U; i < n; i++) {
		x[i].key = i;
		x[i].recid = n - i;
	}
...
void shuffle(rec x[], const unsigned int n) {
...
void quicksort(rec x[], const unsigned int L, const unsigned int R) {
	const auto pivot = x[R].key;
	auto i = L;
	for (auto j = L; j < R; j++) {
		if (x[j].key < pivot) {
			const auto tmp = x[i];
...
/*
 * Given an array x[] of size n elements with sorted groups of size sorted_size, merge each pair of two sorted groups
 * into one in the output array y[]. The result is an array with groups of size 2 * sorted_size.
 */
__global__ void merge(rec x[], rec y[], const unsigned int n, const unsigned int sorted_size) {
	const auto thread_id = blockIdx.x * blockDim.x + threadIdx.x;
	if (thread_id >= n / (2 * sorted_size))
		return;
	const auto left_start = thread_id * 2 * sorted_size, right_start = left_start + sorted_size,
			   output_start = left_start, left_end = right_start,
			   right_end = right_start + sorted_size < n ? right_start + sorted_size : n;
	if (right_start >= n) {
		// Only left side exists, just copy it
		for (auto i = left_start; i < n && i < left_end; i++)
			y[i] = x[i];
		return;
	}
	auto i = left_start, j = right_start, k = output_start;
...
/*
 * This is a primitive merge sort. It is doing a lot of reading and writing to global memory. Also, it does not end
 * early. With a recursive merge, if you notice that left[n] < right[0], you can stop. Here we have to copy every time.
 *
...
/*
 * sortingnetwork8 is an optimal sorting network that will read each group of 8 elements into registers, sort them, and
 * write them back. This is much more efficient than the first few passes of merge sort. Once we see how this works, we
 * can do this in bigger groups (16, 32, 64). We could also try to have multiple threads working on the same optimal
 * sorting network.
 */
__global__ void sortingnetwork8(rec* x, rec* y, const unsigned int n) {
	const auto thread_id = blockIdx.x * blockDim.x + threadIdx.x;
	if (thread_id >= n / 8)
		return; // we only need this many threads
	auto a = x[thread_id * 8], b = x[thread_id * 8 + 1], c = x[thread_id * 8 + 2], d = x[thread_id * 8 + 3],
		 e = x[thread_id * 8 + 4], f = x[thread_id * 8 + 5], g = x[thread_id * 8 + 6], h = x[thread_id * 8 + 7];
	/*
		[(0,2),(1,3),(4,6),(5,7)]
		[(0,4),(1,5),(2,6),(3,7)]
		[(0,1),(2,3),(4,5),(6,7)]
...
/*
 * Given an array x[] of size n elements with sorted groups of size sorted_size, merge each four groups into one in the
 * output array y[]. The result is an  array with groups of size 4 * sorted_size. This skips one pass of merge sort, and
 * more importantly, we are going to try to use local registers to merge. The if statements are still going to diverge
 * though. At the moment, local blocks of size 8, but 16 is probably optimal because CUDA uses blocks of 32 32-bit
 * values for cache line. We are only using 16.
 */
__global__ void merge4(rec x[], rec y[], const unsigned int n, const unsigned int sorted_size) {
	const auto thread_id = blockIdx.x * blockDim.x + threadIdx.x;
	if (thread_id >= n / (4 * sorted_size))
		return;
	const auto g0 = thread_id * 4 * sorted_size, g1 = g0 + sorted_size, g2 = g1 + sorted_size, g3 = g2 + sorted_size,
			   g_end = g3 + sorted_size < n ? g3 + sorted_size : n;
	auto i = g0, j = g1, k = g2, m = g3, out = g0;
	const auto i_end = g1, j_end = g2, k_end = g3, m_end = g_end;
	// 4-way merge from global memory
	while (i < i_end && j < j_end && k < k_end && m < m_end) {
		auto min_key = x[i].key, min_idx = 0U;
		if (x[j].key < min_key) {
			min_key = x[j].key;
			min_idx = 1;
		}
		...
		y[out++] = x[(min_idx == 0 ? i : min_idx == 1 ? j : min_idx == 2 ? k : m)++];
	}
	// finish remaining elements
	while (i < i_end && j < j_end && k < k_end)
		y[out++] = x[(x[i].key < x[j].key ? x[i].key < x[k].key ? i : k : x[j].key < x[k].key ? j : k)++];
	while (i < i_end && j < j_end && m < m_end)
		y[out++] = x[(x[i].key < x[j].key ? x[i].key < x[m].key ? i : m : x[j].key < x[m].key ? j : m)++];
	while (i < i_end && k < k_end && m < m_end)
		y[out++] = x[(x[i].key < x[k].key ? x[i].key < x[m].key ? i : m : x[k].key < x[m].key ? k : m)++];
	while (j < j_end && k < k_end && m < m_end)
		y[out++] = x[(x[j].key < x[k].key ? x[j].key < x[m].key ? j : m : x[k].key < x[m].key ? k : m)++];
...
/*
 * This is an attempt at a more efficient merge sort. We are going to copy into local registers, merge four ways
 * (skipping one generation) then copy back to global memory.
 *
 * NOTE: Kernel launches must be done from host, not from within a kernel.
 */
__global__ void initial_sort(rec x[], const unsigned int n, const unsigned int block_size) {
	const auto thread_id = blockIdx.x * blockDim.x + threadIdx.x;
	const auto start = thread_id * block_size;
	if (start >= n)
		return;
	const auto end = start + block_size < n ? start + block_size : n;
	// Simple bubble sort variant for small blocks (better for GPU than insertion sort)
	for (auto i = start; i < end - 1; i++) {
		for (auto j = start; j < end - 1 - (i - start); j++) {
...
void merge_sort_wrapper(void* ptr, const unsigned int n) {
	...
	extern unsigned int num_threads_global;
	// First, sort initial blocks to reduce number of merge passes
	constexpr auto initial_block_size = 256U;
	const auto num_initial_blocks = (n + initial_block_size - 1) / initial_block_size;
	auto num_blocks_initial = (num_initial_blocks + num_threads_global - 1) / num_threads_global;
	if (num_blocks_initial == 0)
		num_blocks_initial = 1;
	initial_sort<<<num_blocks_initial, num_threads_global>>>(x_gpu, n, initial_block_size);
	auto a = x_gpu, b = y_gpu_global;
	for (auto sorted_size = initial_block_size; sorted_size < n; sorted_size *= 2) {
		const auto num_pairs = n / (2 * sorted_size);
		if (num_pairs == 0)
			break;
		auto num_blocks = (num_pairs + num_threads_global - 1) / num_threads_global;
		if (num_blocks == 0)
			num_blocks = 1;
...
void merge_sort2_wrapper(void* ptr, const unsigned int n) {
	...
	extern rec* y_gpu_global;
	extern unsigned int num_threads_global;
	if (const auto num_groups = n / 8; num_groups > 0) {
		auto num_blocks_sort = (num_groups + num_threads_global - 1) / num_threads_global;
	...
	auto a = x_gpu, b = y_gpu_global;
	for (auto sorted_size = 8U; sorted_size < n; sorted_size *= 4) {
		const unsigned int num_quads = n / (4 * sorted_size);
		if (num_quads == 0)
			break;
		auto num_blocks_merge4 = (num_quads + num_threads_global - 1) / num_threads_global;
...
#if 0
// forget this
__global__ void copySlice(const unsigned int inputArray[], int startIndex, int length) {
...
void check_sorted(const rec* x, const unsigned int n) {
	auto error = 0U;
	...
	const auto max_errors = error > 10 ? 10 : error;
	if (error > 0) {
...
constexpr auto n = 16U * 1024 * 1024; // 64M elements, 512MB data (each element is 8 bytes)
rec* y_gpu_global;
unsigned int num_threads_global;
