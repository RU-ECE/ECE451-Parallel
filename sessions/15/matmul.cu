...
__global__ void set_matrix(float* A, const int N, const float k) {
...
__global__ void transpose_shared(const float a[], float ans[], const int N) {
	...
	const int out_row = blockIdx.x * TILE + threadIdx.x;
	const int out_col = blockIdx.y * TILE + threadIdx.y;
...
__global__ void transpose_shared_coalesced(const float a[], float ans[], const int N) {
	...
	const int x_in = blockIdx.x * TILE + threadIdx.x;
	const int y_in = blockIdx.y * TILE + threadIdx.y;
	...
	const int x_out = blockIdx.y * TILE + threadIdx.x;
	const int y_out = blockIdx.x * TILE + threadIdx.y;
...
__global__ void matmul_tiled_register(const float* A, const float* B, float* C, const int N) {
	...
	const int baseRow = blockIdx.y * TILE;
	const int baseCol = blockIdx.x * TILE;

	const int localRow = threadIdx.y * UNROLL_ROWS;
	const int localCol = threadIdx.x * UNROLL_COLS;
	...
	for (auto t = 0; t < nTiles; ++t) {
		...
		for (int i_sh = threadIdx.y; i_sh < TILE; i_sh += blockDim.y) {
			const int aRow_load = baseRow + i_sh; // Global row index for A
			const int bRow_load = t * TILE + i_sh; // Global row index for B
			for (int j_sh = threadIdx.x; j_sh < TILE; j_sh += blockDim.x) {
				const int aCol_load = t * TILE + j_sh; // Global col index for A
				const int bCol_load = baseCol + j_sh; // Global col index for B
	...
	// write results
	for (auto ir = 0; ir < UNROLL_ROWS; ++ir) {
		const int globalRow = baseRow + localRow + ir;
		for (auto jc = 0; jc < UNROLL_COLS; ++jc) {
			const int globalCol = baseCol + localCol + jc;
...
__host__ void fill_host(float* h, const int N, const float v) {
	for (auto i = 0; i < N * N; ++i)
		h[i] = v;
}

__host__ bool approx_equal(const float* a, const float* b, const int N, const float tol = 1e-3f) {
	for (auto i = 0; i < N * N; ++i) {
		float A = a[i], B = b[i];
...
__host__ void transpose_host(const float* A, float* At, const int N) {
	for (auto r = 0; r < N; ++r)
		for (auto c = 0; c < N; ++c)
			At[c * N + r] = A[r * N + c];
}
...
float benchmark_and_report(const char* name, F launch_and_sync, const double scale_value, const char* unit,
						   int iterations = 10) {
	const float ms = benchmark_kernel(launch_and_sync, iterations);
	const double metric = ms > 0.0f ? scale_value / (ms * 1e-3) : 0.0;
	print_perf_line(name, ms, metric, unit);
	return ms;
}

int main(const int argc, char** argv) {
	const int N = argc >= 2 ? atoi(argv[1]) : 1024;
	printf("N = %d, tile = %d\n", N, TILE);

	const size_t bytes = static_cast<size_t>(N) * N * sizeof(float);
	const auto hA = static_cast<float*>(malloc(bytes));
	const auto hB = static_cast<float*>(malloc(bytes));
	const auto hC = static_cast<float*>(malloc(bytes));
	const auto hCref = static_cast<float*>(malloc(bytes));
	const auto hBt = static_cast<float*>(malloc(bytes));
	...
	constexpr auto REG_UNROLL = 4;
	constexpr auto MATMUL_UC = 2;
	constexpr auto MATMUL_UR = 2;
	...
	const double matmul_work_giga = 2.0 * static_cast<double>(N) * N * N / 1e9;
