 ...
 */
void read_memory(const char a[], const uint64_t n) {
 ...
 */
void read_memory64(const uint64_t a[], const uint64_t n) {
	for (uint64_t i = 0; i < n; i++)
		volatile uint64_t sink = a[i];
}
 ...
 */
void read_memory_skip(const uint64_t a[], const uint64_t n, const uint64_t skip) {
...
int main() {
	...
	constexpr uint64_t n8 = n * 8;
	constexpr uint64_t num_trials = 5;
