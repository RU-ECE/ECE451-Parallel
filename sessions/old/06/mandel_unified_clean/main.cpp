...
#include "mandelbrot_ispc.h" // header generated for vectorized
#include "mandelbrot_tasks_ispc.h" // header generated for multithreaded vectorized
...
void benchmark1(const uint32_t num_trials,
								const char msg[], uint32_t max_iterations,
...
void bench(const uint32_t num_trials, const uint32_t vector_size, uint32_t res, const float x0, const float x1,
	...
	benchmark1(num_trials, "scalar", x0, x1, y0, y1, width, height,
						 max_iterations, counts_serial, mandelbrot_serial);
	benchmark1(num_trials, "vectorized", x0, x1, y0, y1, width, height,
						 max_iterations, counts_parallel, mandelbrot_ispc);
	...
	benchmark1(num_trials, "parallel", x0, x1, y0, y1, width, height,
						 max_iterations, counts_parallel, mandelbrot_tasks_ispc);
	verify(counts_serial, counts_parallel, num_pixels);
