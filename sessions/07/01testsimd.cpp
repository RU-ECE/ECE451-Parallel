...
void product(const float* a, const float* b, float* c, const int n) {
	for (auto i = 0; i < n; i += 8) { // think in terms of floats, we do 0..7, 8..15, ...
		auto tempa = _mm256_loadu_ps(a + i); // first time a+0 = a, second time a + 8*sizeof(float)
		const auto tempb = _mm256_loadu_ps(b + i);
...
float dotproduct_vector(const float* a, const float* b, const int n) {
	auto sum = _mm256_setzero_ps(); // set all 8 floats to 0
	...
	const auto temp1 = _mm256_hadd_ps(sum, sum);
	auto temp2 = _mm256_hadd_ps(temp1, temp1);
	const auto low = _mm256_castps256_ps128(temp2);
	const auto high = _mm256_extractf128_ps(temp2, 1);
	const auto final_sum = _mm_add_ps(low, high);
	const auto fsum = _mm_cvtss_f32(final_sum);
	return fsum;
}
...
int main() {
	constexpr auto n = 1024UL * 1024 * 128;
	...
	auto dot = dotproduct_scalar(a, b, n);
