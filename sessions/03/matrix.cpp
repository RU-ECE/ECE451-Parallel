...
private:
    ...
    matrix(uint32_t rows, uint32_t cols, const char* internal)
     : rows(rows), cols(cols), m(new float[rows*cols]) {}
public:
    matrix(uint32_t rows, uint32_t cols) : rows(rows), cols(cols), m(new float[rows*cols]) {
    ...
    float& operator()(uint32_t r, uint32_t c) {
        return m[r*rows + c];
    }

    // this is the readonly version: cout << m(i,j)
    float operator()(uint32_t r, uint32_t c) const {
        return m[r*rows + c];
    }
...
void benchmark() {
    ...
    matrix a = matrix::ident(1024);
    matrix b = matrix::ident(1024);
    matrix c = a * b;
