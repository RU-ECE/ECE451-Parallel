...
public:
	explicit prime_bits(const uint64_t n) : // round up because we need n+1 bits
	...
	// use bitcounting to avoid counting each one separately
	uint64_t fast2_eratosthenes() const {
		uint64_t count = 1; // 2 is a special case
		uint64_t lim = sqrt(n);
		uint64_t k = 0;
	...
	// use bitcounting to avoid counting each one separately
	uint64_t segmented_eratosthenes(const uint64_t blocksize) const {
		uint64_t count = eratosthenes(sqrt(n));
		const uint64_t start = (sqrt(n) + 127) / 128; // round up to next word location
		[[maybe_unused]] uint64_t end = start + blocksize;
		uint64_t lim = sqrt(n);
		uint64_t k = 0;
		for (uint64_t i = 0; i < num_words; i++) {
			//			p[i] = wheel[k++];
