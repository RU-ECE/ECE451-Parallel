...
double sum(const __m256d a[], const __m256d b[], const int n) {
	...
	for (auto i = 0; i < n; i += 4) {
		const __m256d temp = _mm256_add_pd(a[i], b[i]);
	...
	const double s = sum[0] + sum[1] + sum[2] + sum[3];
	return s;
}


int main() {
	constexpr uint64_t a = 1; // mov $1, %rax
	...
	const double s1 = sum(x, y, n);
	const double s2 = sum(reinterpret_cast<__m256d*>(x), reinterpret_cast<__m256d*>(x), n);
	cout << "s1=" << s1 << endl;
	cout << "s2=" << s2 << endl;
}
