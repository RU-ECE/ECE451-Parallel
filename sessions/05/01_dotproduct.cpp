...
double dot_product(const __m256d a[], const __m256d b[], const int n) {
	...
	for (auto i = 0; i < n / 4; i++) {
		const __m256d temp = _mm256_mul_pd(a[i], b[i]);
...
int main() {
	...
	const double s1 = dot_product(x, y, n);
	auto t1 = high_resolution_clock::now();
	auto elapsed = duration_cast<microseconds>(t1 - t0);
	cout << "s1=" << s1 << "\telapsed: " << elapsed.count() << "usec\n";

	t0 = high_resolution_clock::now();
	const double s2 = dot_product(reinterpret_cast<const __m256d*>(x), reinterpret_cast<const __m256d*>(y), n);
