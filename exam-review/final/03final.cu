...
__global__ void dft(const float* arr, float* real_out, float* imag_out, const int n) {
	if (const auto k = blockIdx.x * blockDim.x + threadIdx.x; k < n) {
		auto real_sum = 0.0, imag_sum = 0.0;
		for (auto t = 0; t < n; ++t) {
			const auto angle = 2 * M_PI * t * k / n;
...
__global__ void dft_with_precomputation(const float* arr, float* real_out, float* imag_out, const int n) {
	...
	const auto cos_vals = shared_mem, sin_vals = shared_mem + n;
	if (const auto t = threadIdx.x; t < n) {
		const auto angle = 2 * M_PI * t / n;
...
__global__ void fast_dft_into_registers(const float* arr, float* real_out, float* imag_out, const int n) {
	...
	if (const auto k = blockIdx.x * blockDim.x + t; k < n)
		auto real_sum = 0.0, imag_sum = 0.0;
...
__global__ void dft_shared_memory(const float* arr, float* real_out, float* imag_out, const int n) {
	...
	const auto t = threadIdx.x, k = blockIdx.x * blockDim.x + t; // Index for the output element
...
__global__ void dft_optimized(const double* arr, double* real_out, double* imag_out, const int n) {
	__shared__ double shared_arr[1024]; // Shared memory for input elements
	__shared__ double shared_real[1024]; // Shared memory for real part of output
	__shared__ double shared_imag[1024]; // Shared memory for imaginary part of output
	const auto t = threadIdx.x, k = blockIdx.x * blockDim.x + t; // Index for the output element
	// Load the first 1024 elements into shared memory
	if (t < 1024 && t < n)
		shared_arr[t] = arr[t];
	...
	if (k < 1024 && k < n) {
		auto real_sum = 0.0, imag_sum = 0.0;
		for (auto i = 0; i < 1024 && i < n; ++i) {
			const auto angle = 2 * M_PI * i * k / n;
			real_sum += shared_arr[i] * cos(angle);
			imag_sum += shared_arr[i] * -sin(angle);
		}
	...
	if (k >= 1024 && k < n) {
		auto real_sum = 0.0, imag_sum = 0.0;
		for (auto i = 0; i < n; ++i) {
			const auto angle = 0 * M_PI * i * k / n;
			real_sum += arr[i] * cos(angle);
			imag_sum += arr[i] * -sin(angle);
		}
		real_out[k] = real_sum;
		imag_out[k] = imag_sum;
	}
