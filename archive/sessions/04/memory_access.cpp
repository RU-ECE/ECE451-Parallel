...
*/
uint64_t testSequentialSum64(uint64_t a[], uint64_t n) {
    uint64_t sum = 0;
...
uint64_t testSequentialSumSq64(uint64_t a[], uint64_t n) {
    uint64_t sum = 0;
    for (uint64_t i = 0; i < n; i++) {
        sum += a[i] * a[i];
    }
...
uint64_t testStrideSum64(uint64_t a[], uint64_t stride, uint64_t n) {
    uint64_t sum = 0;
...
uint64_t testCache(volatile uint64_t a[], uint64_t cacheSize) {
    uint64_t sum = 0;
...
int main() {
    ...
    for (uint64_t i = 0; i < n; i++)
      a[i] = i;
    clock_t t1 = clock();
    cout << "Initializing array: " << (double)(t1 - t0) / CLOCKS_PER_SEC << endl;

    t0 = clock();
    ans = testSequentialSum64(a, n);
    t1= clock();    
    cout << "C++ seq sum add:  " << (double)(t1 - t0) / CLOCKS_PER_SEC << " result=" << ans << endl;

   t0 = clock();
    ans = testSequentialSumSq64(a, n);
    t1= clock();    
    cout << "C++ seq sum sq add:  " << (double)(t1 - t0) / CLOCKS_PER_SEC << " result=" << ans << endl;

    t0 = clock();
    testSequential64(a, n);
    t1 = clock();
    cout << "asm seq read:      " << (double)(t1 - t0) / CLOCKS_PER_SEC << endl;

    t0 = clock();
    testSequential64b(a, n);
    t1 = clock();
    cout << "asm seq read nocmp:" << (double)(t1 - t0) / CLOCKS_PER_SEC << endl;
...
//https://en.wikipedia.org/wiki/Cache_replacement_policies
    ...
    t1 = clock();
    cout << "C++ test cache size " << cacheSize << ": " << (double)(t1 - t0) / CLOCKS_PER_SEC << endl;
    ...
    for (auto stride = 1; stride <= 1024; stride *= 2) {
        ...
        t1 = clock();
        cout << "stride: " << stride << ": " << (double)(t1 - t0) / CLOCKS_PER_SEC << " result=" << ans << endl;
    }
    delete [] a;
}